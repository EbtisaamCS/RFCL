...
class KMeansAggregator(Aggregator):
    ...
    def trainAndTest(self, testDataset: DatasetInterface) -> Errors:
        ...
        for r in range(self.config.rounds):
            ...
            with torch.no_grad():
                ...
                if self.personalisation == "GENERAL":
    ...
    def _use_most_similar_clustersmnist(self) -> Tuple[List[nn.Module], Tensor, List[int]]:
        """
    ...
    def _use_most_similar_clustersgood(self) -> Tuple[List[nn.Module], Tensor, List[int]]:
        """
    ...
    def __elbow_test(self, models: List[nn.Module]) -> None:
        ...
        #X = self._generate_weights(models)
        X = []
        for model in self.model.parameters():
            flattened_params = torch.tensor([]).to(self.device)
            for param in model.view(-1):
                flattened_params = torch.cat((flattened_params, param.view(-1)), dim=0)
            X.append(flattened_params.numpy())
        ...
        for h in range(len(self.clients)):
            ...
            d = 0
            for i, ins in enumerate(indices):
                centres.append(self._gen_cluster_centre(ins, self.models))

            for i, ins in enumerate(indices):
                ms = [self.clients[j].model for j in ins]
                c_coords = torch.tensor([]).to(self.device)
                for param in centres[i].parameters():
                    c_coords = torch.cat((c_coords, param.view(-1)),dim=0)

                for m in ms:
                    m_coords = torch.tensor([]).to(self.device)
                    for param in m.parameters():
                        m_coords = torch.cat((m_coords, param.view(-1)),dim=0)
        ...
        plt.figure()
        plt.plot(range(1, len(self.clients) + 1), dispersion)
        plt.title(
    ...
    def __elbow_testtt(self, models: List[nn.Module]) -> None:
        """
        This is a helper function for calculating the optimum K.
        ...
        It uses the sum of distances away from a cluster centre to determine if K is too big or small.
        """
        X = self._generate_weights(self.models)
        dispersion = []
